...
public interface UserToDoRepository extends JpaRepository<UserToDo, Long> {
        ...
        @Query("select ut from ToDo t inner join UserToDo ut on ut.toDo.id = t.id where ut.user.id = :userId AND t.deleteState = false AND t.completion = false AND t.endDate >= :today AND t.startDate <= :today")
